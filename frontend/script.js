/**
 * YouTube Downloader Pro - Enhanced Frontend (Fixed)
 */
class YouTubeDownloaderUI {
    ...
    initElements() {
        this.elements = {
            ...
            speedText: document.getElementById('speedText'),       // di progress info
            etaText: document.getElementById('etaText'),           // di progress info
            sizeText: document.getElementById('sizeText'),         // di progress info
            downloadedText: document.getElementById('downloadedText'), // di progress info
            // Status grid elements (UNIQUE — no duplicates!)
            speedValue: document.getElementById('speedValue'),
            etaValue: document.getElementById('etaValue'),
            sizeValue: document.getElementById('sizeValue'),
            downloadedValue: document.getElementById('downloadedValue'),
            // Progress bar
    ...
    async checkServerBusy() {
        try {
            ...
            const data = await response.json();
            if (data.busy) {
                this.showNotification('Server sedang sibuk, memulai polling status...', 'warning');
    ...
    async startDownload() {
        ...
        if (this.isDownloading) {
            this.showNotification('Download sedang berjalan', 'warning');
            return;
        }
        ...
        if (this.elements.statusIndicator) {
            ...
            const label = this.elements.statusIndicator.querySelector('.status-label');
            if (label) label.textContent = 'Mengunduh';
        }

        // JANGAN hapus URL input — biarkan user lihat URL yang didownload
        ...
        this.statusInterval = setInterval(async () => {
            ...
            try {
                ...
                if (status.status === 'completed') {
                    ...
                    clearInterval(this.statusInterval);
                    // Jangan otomatis reset — biarkan user lihat hasil
                }
                    ...
                    // Jika idle tapi UI masih menunjukkan downloading
    ...
    updateProgress(status) {
        ...
        // Update progress info (speed, ETA, size)
        if (status.speed && status.speed !== '0 KB/s') {
            if (this.elements.speedText) this.elements.speedText.textContent = `Kecepatan: ${status.speed}`;
        }
        if (status.eta && status.eta !== '--:--') {
            if (this.elements.etaText) this.elements.etaText.textContent = status.eta;
        }
        if (status.filesize && status.filesize !== '0 MB') {
            if (this.elements.sizeText) this.elements.sizeText.textContent = status.filesize;
        }
        if (status.downloaded && status.downloaded !== '0 MB') {
            if (this.elements.downloadedText) this.elements.downloadedText.textContent = status.downloaded;
        }

        // Update status GRID (unique elements)
        if (status.speed && status.speed !== '0 KB/s') {
            if (this.elements.speedValue) this.elements.speedValue.textContent = status.speed;
        }
        if (status.eta && status.eta !== '--:--') {
            if (this.elements.etaValue) this.elements.etaValue.textContent = status.eta;
        }
        if (status.filesize && status.filesize !== '0 MB') {
            if (this.elements.sizeValue) this.elements.sizeValue.textContent = status.filesize;
        }
        if (status.downloaded && status.downloaded !== '0 MB') {
            if (this.elements.downloadedValue) this.elements.downloadedValue.textContent = status.downloaded;
        }
    ...
    resetUI() {
        ...
        // Reset progress info
        if (this.elements.speedText) this.elements.speedText.textContent = 'Kecepatan: 0 KB/s';
        if (this.elements.etaText) this.elements.etaText.textContent = '--:--';
        if (this.elements.sizeText) this.elements.sizeText.textContent = '0 MB';
        if (this.elements.downloadedText) this.elements.downloadedText.textContent = '0 MB';

        // Reset status grid
        if (this.elements.speedValue) this.elements.speedValue.textContent = 'Menunggu...';
        if (this.elements.etaValue) this.elements.etaValue.textContent = '--:--';
        if (this.elements.sizeValue) this.elements.sizeValue.textContent = '0 MB';
        if (this.elements.downloadedValue) this.elements.downloadedValue.textContent = '0 MB';
        ...
        // JANGAN kosongkan URL input — UX lebih baik biar user lihat riwayat URL

        this.updateStatus('Masukkan URL YouTube untuk memulai download', 'info');
    }

    isValidYouTubeURL(url) {
        // Regex yang lebih ketat: validasi domain dan adanya path
        const youtubeRegex = /^(https?:\/\/)?(www\.|m\.)?(youtube\.com\/(watch\?v=|embed\/|shorts\/|playlist\?list=)|youtu\.be\/).+/;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.youtubeDownloader = downloader;

    // Add some initial animation
    const mainCard = document.querySelector('.main-card');
    if (mainCard) {
        mainCard.style.animation = 'fadeIn 0.8s ease-out';
    }
});
